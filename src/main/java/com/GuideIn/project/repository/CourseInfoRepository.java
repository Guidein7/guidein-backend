...
public interface CourseInfoRepository extends JpaRepository<CourseInfo,Long>{
	...
	@Query(value="select * from course_info where ?1 is null or location =?1 and ?2 is null or mode_of_class=?2 and ?3 is null or course_duration=?3",nativeQuery=true)
	Page<CourseInfo> findByFilters(String location, String modeOfClass,String duration, Pageable pageable);
